...
| `[~]` | Booleans, pairs/lists, symbols, and equivalence predicates (6.1, 6.3.1-6.3.3, pp. 17-19, 25-29) | Partial | `Value`, `Pair`, list helpers, symbol conversion, and cycle-aware `equal?`; source symbols are case-folded while `string->symbol` preserves string-created spelling identity. | Mutation, improper-list domain errors, selectors, membership/association, structural equality, symbol conversion case, immutable `symbol->string` results, and cycle termination are covered by `R5RSIODataTests`; direct unspecified `eq?`/`eqv?` corner cases and full identity matrix remain open. |
| `[~]` | Characters and strings (6.3.4-6.3.5, pp. 29-31) | Partial | Character reader and character/string primitives backed by Swift `Character`. | `R5RSIODataTests.characterCaseConversion` covers ASCII case invariants, scalar-safe `char-upcase`/`char-downcase`, case-insensitive comparison, and integer round trips; `stringCaseOrderingUsesCharacterKeys` proves `string-ci` extends the same scalar case classes as `char-ci` across equality and ordering. The compact policy preserves expansion-prone Unicode mappings (for example `ß`/`İ`) as scalar-safe characters; Unicode repertoire/order and index/error boundaries still need a linked matrix. |
...
| `[~]` | Control features, multiple values, promises, `eval` (6.4-6.5, pp. 31-35) | Partial | Special procedures for `apply`, `call/cc`, `values`, `call-with-values`, `dynamic-wind`, `force`, and `eval`; environment constructors. | Existing tests cover escapes, multiple values, wind transitions, promise memoization, and report-environment evaluation. `R5RSDerivedControlTests.emptyListProcedureDomains` also proves `map`/`for-each` reject non-procedures even when the list is empty. The value-count/error matrix remains open. |
