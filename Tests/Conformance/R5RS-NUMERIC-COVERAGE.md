...
| `[~]` | Writer/string conversion and radix 2/8/10/16 read-back | `R5RSNumericTests` grammar tables; `numeric library`; `bignum number string round trip`; BigInt kernel radix invariants; `radixAcceptsExactComplexInteger` | Add a complete round-trip matrix for every numeric kind. |
