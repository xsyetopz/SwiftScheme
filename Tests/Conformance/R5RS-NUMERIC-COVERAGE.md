...
|---|---|---|---|
| `[~]` | Exact integers: signs, arbitrary decimal magnitude, `#b/#o/#d/#x`, prefix order | `R5RSNumericTests.radixPlaceholders`, `validNumericForms`, and `arbitraryExactExponents`; `numeric library`; `arbitrary exact integer`; BigInt kernel `arithmetic, division, radix, and conversion invariants` | Full arbitrary-radix boundary/error matrix remains open; `arbitraryExactExponents` preserves exact unit-base powers beyond `Int`, while unsupported huge non-unit exact exponents report a numeric restriction rather than silently becoming inexact. |
| `[~]` | Exact rationals: normalization, negative denominator, zero rejection, arbitrary operands | `rational and complex arithmetic`; `exact literals`; `R5RSNumericTests.rationalAccessorBoundaries` | Broader arbitrary-operand and reader error cases remain open. |
...
| `[~]` | Inexact reals: decimal point, `e/s/f/d/l` suffixes, `#i`, placeholders, signed zero | `R5RSNumericTests.radixPlaceholders`, `validNumericForms`, and malformed forms; `numericPredicateCrossProduct`; `round ties to even`; `integer procedures accept inexact integers` | Signed zero, infinity, NaN, and undefined `log 0` conventions remain open; do not infer coverage from the parser alone. |
...
| `[~]` | Writer/string conversion and radix 2/8/10/16 read-back | `R5RSNumericTests` grammar tables; `numeric library`; `bignum number string round trip`; BigInt kernel radix invariants; `radixAcceptsExactComplexInteger`; `numberStringFiniteInexactNonDecimalRoundTrips`; `numberStringFiniteInexactBoundaryRoundTrips` | Exact/inexact finite round-trip boundaries, including binary64 subnormal/normal/extreme values, are covered for representative radices; add a complete matrix for every numeric kind and special-value convention. |
...
| `[~]` | Rounding: `floor`, `ceiling`, `truncate`, `round`, exactness | `R5RSProcedureSemanticsTests.roundingContracts` proves nearest-even ties, negative ties, and exact/inexact result classes. | Add broader negative, infinity/NaN, and complex-domain cases. |
| `[~]` | Transcendentals: `exp`, `log`, `sin`, `cos`, `tan`, `asin`, `acos`, `atan` | Registered and implemented numeric primitive paths; procedure inventory; `signedZeroBranchCuts`; `transcendentalOverflowRetainsRealDomain` | Add real/complex branch, domain, and special-value cases. |
...
| `[~]` | Complex construction/accessors: `make-rectangular`, `make-polar`, `real-part`, `imag-part`, `magnitude`, `angle` | `exact square roots and magnitudes` covers magnitude only; `R5RSNumericTests.complexConstructionAccessors` covers constructors/accessors and the principal-angle range | Broader polar magnitude/angle and non-finite cases remain open. |
| `[~]` | Exactness conversion: `exact->inexact`, `inexact->exact` for real/complex values | Registered and implemented numeric primitive paths; procedure inventory; `exactToInexactRoundsLargeIntegers`; `numberStringFiniteInexactBoundaryRoundTrips` | Large-integer nearest-even and finite binary64 conversion boundaries are covered; add complex, binary64 overflow/underflow, and error cases. |
