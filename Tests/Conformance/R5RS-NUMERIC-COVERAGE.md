...
| `[~]` | Writer/string conversion and radix 2/8/10/16 read-back | `R5RSNumericTests` grammar tables; `numeric library`; `bignum number string round trip`; BigInt kernel radix invariants; `radixAcceptsExactComplexInteger`; `numberStringFiniteInexactNonDecimalRoundTrips` | Exact/inexact finite round-trip boundaries are covered for representative values; add a complete matrix for every numeric kind and special-value convention. |
